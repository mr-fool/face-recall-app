...
// Debug helper - log to ensure file is loading
console.log('Renderer.js loading...');

// Add debugging for buttons
document.addEventListener('DOMContentLoaded', () => {
  console.log('DOM fully loaded - adding direct click handlers');
  
  // Debug all tab buttons
  ...
  allTabButtons.forEach((btn, index) => {
    ...
    // Force add a click handler directly
    btn.onclick = function(e) {
      console.log('Button clicked via direct onclick:', this.textContent);
      ...
      if (panel) {
        ...
        console.log('Activated tab panel:', tabName);
      } else {
        console.error('Could not find tab panel with ID:', tabName);
      }
    };
  });
});

// Load database module with proper initialization
let database;
try {
  database = require('./modules/database');
  
  // Get user data path from main process and initialize the database
  (async function initDb() {
    try {
      const userDataPath = await ipcRenderer.invoke('get-user-data-path');
      database.init(userDataPath);
      console.log('Database initialized with user data path:', userDataPath);
      
      // Now that DB is initialized, load saved people
      loadSavedPeople();
    } catch (error) {
      console.error('Failed to initialize database with user data path:', error);
    }
  })();
} catch (error) {
  ...
  database = {
    init: () => true,
    getAllPeople: () => Promise.resolve([]),
    addPerson: (person) => Promise.resolve(person),
    updatePerson: () => Promise.resolve(1),
    deletePerson: () => Promise.resolve(1),
    updateLastRecognized: () => Promise.resolve(1)
  };
}
...
async function init() {
  try {
    ...
    // Configure tab navigation first - this should work without any dependencies
    setupTabNavigation();
    
    // Setup other event listeners that don't depend on face-api
    setupBasicEventListeners();
    
    // Apply settings that don't depend on face-api
    applySettings();
    
    console.log('Basic initialization complete. Waiting for face-api to load...');
  } catch (error) {
    console.error('Error during app initialization:', error);
  }
}

// Setup tab navigation specifically
...
async function toggleCamera() {
  ...
  if (isCameraActive) {
    ...
    try {
      ...
      // Set up camera overlay for drawing face boxes
      if (cameraOverlay) {
        cameraOverlay.width = cameraView.videoWidth || 640;
        cameraOverlay.height = cameraView.videoHeight || 480;
      }
...
async function recognizeFace() {
  if (!isCameraActive || recognitionActive || !faceapi) {
    console.warn('Cannot recognize face: Camera inactive or recognition in progress or faceapi not loaded');
    return;
  }
  
  recognitionActive = true;
  if (loadingIndicator) loadingIndicator.classList.remove('hidden');
  if (recognitionDisplay) recognitionDisplay.classList.add('hidden');
  if (noRecognition) noRecognition.classList.add('hidden');
  ...
  try {
    ...
    if (detections.length === 0) {
      ...
      if (noRecognition) noRecognition.classList.remove('hidden');
    ...
    const threshold = parseFloat(confidenceThreshold ? confidenceThreshold.value : 0.6);
    ...
    if (match.label !== 'unknown') {
      ...
      if (personName) personName.textContent = person.name;
      if (personRelation) personRelation.textContent = person.relationship || 'No relationship specified';
      if (personNotes) personNotes.textContent = person.notes || 'No additional notes';
      
      // Show confidence level
      const confidence = Math.round(match.distance * 100);
      if (recognitionConfidence) recognitionConfidence.textContent = `Confidence: ${confidence}%`;
      
      // If person has images saved, show the first one
      if (person.images && person.images.length > 0 && personPhoto) {
      ...
      if (recognitionDisplay) recognitionDisplay.classList.remove('hidden');
    } else {
      if (noRecognition) noRecognition.classList.remove('hidden');
    }
...
async function loadSavedPeople() {
  try {
    console.log('Loading saved people from database...');
    knownPeople = await database.getAllPeople();
    console.log(`Loaded ${knownPeople.length} people from database`);
    updatePeopleList();
    return true;
  } catch (error) {
...
async function selectPhotos() {
  if (!faceapi) {
    alert('Face recognition is not yet loaded. Please wait a moment and try again.');
    return;
  }
    ...
    if (filePaths.length > 0) {
      ...
      for (const filePath of selectedPhotos) {
        try {
          ...
          if (photoPreviewContainer) photoPreviewContainer.appendChild(previewContainer);
        } catch (error) {
          ...
          if (photoPreviewContainer) photoPreviewContainer.appendChild(errorContainer);
        }
      }
    }
  } catch (error) {
...
async function savePerson(event) {
  ...
  const newPerson = {
    ...
    relationship: personRelationshipInput ? personRelationshipInput.value.trim() : '',
    notes: personNotesInput ? personNotesInput.value.trim() : '',
...
function loadSettings() {
 ...
 if (settings.confidenceThreshold && confidenceThreshold) {
 ...
 if (settings.announcementMode && announcementMode) {
   announcementMode.value = settings.announcementMode;
 }
 
 if (settings.textSize && textSize) {
   textSize.value = settings.textSize;
   applyTextSize();
 }
 
 if (settings.highContrast !== undefined && highContrast) {
...
function saveSettings() {
 const settings = {
   confidenceThreshold: confidenceThreshold ? confidenceThreshold.value : 0.6,
   announcementMode: announcementMode ? announcementMode.value : 'name',
   textSize: textSize ? textSize.value : 'normal',
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
 console.log('Document already loaded - starting application immediately');
 setTimeout(init, 0);
}